...
function ImageHolder({ imagePath, label, fill = false }: { imagePath: string; label: string; fill?: boolean }) {
  ...
  return (
    <div className={`relative w-full overflow-hidden bg-slate-100 ${fill ? "min-h-0 flex-1" : "aspect-[4/3]"}`}>
      ...
      {remote && isVideo ? (
        <span className="pointer-events-none absolute bottom-2 left-2 rounded-full bg-ink/70 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-white backdrop-blur">
          Video
        </span>
      ) : null}
    </div>
