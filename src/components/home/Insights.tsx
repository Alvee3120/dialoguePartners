...
function LargeCard({ pillar }: { pillar: Pillar }) {
  return (
    <Link
      ...
      className="group relative flex h-full min-h-[22rem] flex-col justify-end overflow-hidden rounded-3xl border border-line bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl motion-reduce:hover:translate-y-0"
    >
...
function TintedCard({ pillar }: { pillar: Pillar }) {
  return (
    <Link
      href={`/insights/${pillar.slug}`}
      className="group flex flex-col justify-between rounded-3xl bg-gradient-to-br from-card to-accent-soft/40 p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl motion-reduce:hover:translate-y-0"
    >
      <div>
